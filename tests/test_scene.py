...
def test_signal_class_mismatch_blocks_connection(scene, library):
    ...
    scene.add_component_at("ClkGen", 0, 0)
    ...
    # Simulate the start of a drag from src_port, then check the target is rejected
    scene._connection_source_port = src_port
    scene._connection_source_component_id = src_inst.id
    scene._connection_source_interface_port = None

    is_valid = scene._is_valid_connection_target(tgt_port)
    assert not is_valid
